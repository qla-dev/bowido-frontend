...
$lockFile = sys_get_temp_dir().DIRECTORY_SEPARATOR.'trackpal-frontend-redeploy.lock';
...
if ($lock === false || ! flock($lock, LOCK_EX | LOCK_NB)) {
    ...
    $write("Another TrackPal frontend redeploy is already running.\n");
    exit(1);
}
...
$commands = [
    ['label' => 'Pulling latest TrackPal frontend code', 'command' => 'git pull --ff-only origin main'],
    ['label' => 'Installing frontend dependencies', 'command' => $npmCommand.' ci --no-audit --no-fund'],
    ['label' => 'Building TrackPal frontend', 'command' => $npmCommand.' run build'],
];
...
$write("\nTrackPal frontend redeploy completed successfully in ".(time() - $startedAt)."s.\n");
