...
const emptyForm: FormState = { company_name: '', kvk: '', fixed_phone: '', billing_email: '', street: '', postal_code: '' };
...
export function CustomerDetailsPage() {
  ...
  useEffect(() => { void apiService.clients.me().then((detail) => {
    if (detail) setForm({ company_name: detail.name, kvk: detail.kvk_number || '', fixed_phone: detail.fixed_phone || detail.phone_number || '', billing_email: detail.billing_email || '', street: detail.street || detail.delivery_address || '', postal_code: detail.postal_code || '' });
  ...
  return <Card title={t('completeDetails')} className="w-full max-w-none dark:bg-[#101715]">
    {loading ? <p className="py-12 text-center text-sm text-zinc-400">{t('loading')}</p> : <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      ...
      <p className="sm:col-span-2 xl:col-span-3 text-xs font-semibold text-zinc-500 dark:text-zinc-300">{t('warehouse')}: enter your warehouse street and number, then its postal code.</p>
      <label className="space-y-2"><span className="text-xs font-bold dark:text-zinc-200">Street and number</span><Input required value={form.street} onChange={e => setForm({...form, street: e.target.value})} /></label>
