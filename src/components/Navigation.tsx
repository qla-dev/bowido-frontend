...
const NavItem: React.FC<NavItemProps> = ({ icon, label, isActive, onClick, collapsed }) => (
  <button
    ...
    className={cn(
      ...
      isActive ? "text-white" : "text-zinc-500 hover:bg-emerald-50 hover:text-emerald-700"
    )}
  >
    {isActive && (
      <motion.div
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <div className="relative">
            <button
              ...
              className="relative h-10 w-10 border border-emerald-100 bg-white text-zinc-700 rounded-xl flex items-center justify-center hover:border-emerald-300 hover:text-emerald-700 transition-all"
            >
              <Bell size={18} />
              {unreadCount > 0 && (
            ...
            <AnimatePresence>
              {showNotifications && (
                <motion.div
                  ...
                  className="absolute right-0 top-full mt-3 w-[min(21rem,calc(100vw-2rem))] bg-white border border-emerald-100 rounded-2xl shadow-2xl shadow-emerald-950/10 overflow-hidden"
                >
                  <div className="px-4 py-3 border-b border-emerald-50 bg-emerald-50/60">
                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-700">{t('activity')}</span>
                  </div>
                  <div className="max-h-[280px] overflow-y-auto no-scrollbar">
                    {notifications.length === 0 ? (
                      <div className="p-8 text-center text-zinc-300">
                        <p className="text-[9px] font-black uppercase">Clean Inbox</p>
                      </div>
                    ) : (
                      notifications.map(n => (
                        <button
                          ...
                          className={cn(
                            "w-full text-left p-3 border-b border-emerald-50 last:border-0 hover:bg-emerald-50/60 transition-colors",
                            !n.read && "bg-emerald-50/70"
                          )}
                        >
                          <p className="text-[10px] font-black text-zinc-900 uppercase leading-tight mb-0.5">{n.title}</p>
                          <p className="text-[9px] font-bold text-zinc-500 leading-tight mb-1">{n.message}</p>
          ...
          <div className="relative">
            <button
              ...
              className="h-10 pl-1 pr-2 sm:pr-3 border border-emerald-100 bg-white rounded-xl flex items-center gap-2 hover:border-emerald-300 transition-all"
            >
              ...
              <span className="hidden md:block max-w-32 truncate text-[10px] font-black uppercase text-zinc-900">{user.name}</span>
            </button>
            ...
            <AnimatePresence>
              {showProfile && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0, scale: 1 }}
                  exit={{ opacity: 0, y: 6, scale: 0.98 }}
                  className="absolute right-0 top-full mt-3 w-64 bg-white border border-emerald-100 rounded-2xl shadow-2xl shadow-emerald-950/10 overflow-hidden"
                >
                  <div className="p-4 flex items-center gap-3 border-b border-emerald-50">
                    <UserCircle size={34} className="text-emerald-600" />
                    <div className="min-w-0">
                      <p className="text-[11px] font-black uppercase text-zinc-950 truncate">{user.name}</p>
                  ...
                  <button
                    type="button"
                    onClick={onLogout}
                    className="w-full px-4 py-3 flex items-center gap-2 text-rose-500 hover:bg-rose-50 transition-colors"
                  >
                    <LogOut size={16} />
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, role }) => {
  ...
  return (
    <>
      <aside
        id="desktop-sidebar"
        className={`hidden md:flex flex-col border-r border-zinc-200 bg-white h-[calc(100vh-4rem)] sticky top-16 transition-all duration-300 ${
...
export const BottomNav: React.FC<SidebarProps> = ({ activeTab, setActiveTab, role }) => {
  ...
  return (
    <nav
      ...
      className="md:hidden fixed bottom-5 left-4 right-4 bg-white/95 backdrop-blur-2xl border border-zinc-200 p-1.5 rounded-2xl flex justify-around items-center shadow-2xl shadow-zinc-200/70 z-50 overflow-hidden"
    >
      {getNavItems().map((item) => (
        <button
          key={item.id}
