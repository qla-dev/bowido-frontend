...
interface ScannerProps {
  onClose: () => void;
  currentUser: { id: number; name: string; role_id: number };
}
...
export const PalletScanner: React.FC<ScannerProps> = ({ onClose, currentUser }) => {
  ...
  // RBAC: Ograničavanje statusa na osnovu uloge
  const getAllowedStatusIds = () => {
    switch (currentUser.role_id) {
      case 2: // VOZAC (NL)
        return [4, 5, 3];
      case 3: // MAGACINER (BiH/NL)
        return [1, 2, 3, 4, 6, 7];
      case 5: // SERVISER (Doboj)
        return [1];
      default: // ADMIN (1) ili ostali
  ...
  const handleComplete = () => {
    scannedCodes.forEach(code => {
      const pallet = pallets.find(p => p.qr_code === code);
      if (pallet) {
        updatePalletStatus(
          pallet.id,
          selectedStatusId,
              ...
              <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-6">
                   <Button 
                     variant="ghost"
                     size="sm"
                     onClick={() => { setScanMode(null); setScannedCodes([]); }}
                         ...
                         {clients.filter(c => c.name.toLowerCase().includes(clientSearch.toLowerCase())).map(c => (
                           <button
                             ...
                             className={cn(
                               ...
                               selectedClientId === c.user_id 
                                 ? 'border-black bg-white shadow-lg shadow-black/5' 
                                 : 'border-transparent bg-zinc-50/50 text-zinc-400 hover:border-zinc-200'
                             )}
                           >
                              <div>
                                 <p className="text-[10px] font-black text-black uppercase tracking-tight">{c.name}</p>
                                 <p className="text-[8px] font-bold text-zinc-300 uppercase tracking-widest">{c.country}</p>
                              </div>
                              {selectedClientId === c.user_id && <CheckCircle2 size={14} className="text-black" />}
                           </button>
                         ))}
                      </div>
                   </div>
                 ...
                 <div className="space-y-3">
                    ...
                    <Input 
                      placeholder="e.g. Eindhoven Hub, Truck BIH-123" 
                      value={location}
                        ...
                        <div className="flex flex-col">
                          {scannedCodes.map((code) => (
                            ...
                            >
                              ...
                              <Button variant="ghost" size="xs" onClick={() => setScannedCodes(prev => prev.filter(c => c !== code))}>
